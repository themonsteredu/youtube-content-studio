...
const fontFamilies: Record<ContentDraft["fontStyle"], string> = {
...
const fontWeights: Record<ContentDraft["fontStyle"], number> = { strong: 900, rounded: 700, serif: 800, modern: 500 };
...
export function ThumbnailPreview({ draft }: { draft: ContentDraft }) {
  const stageStyle = {
    ...
    fontFamily: fontFamilies[draft.fontStyle],
  };
    ...
    <div className="preview-frame">
      <div className={`thumb-stage thumb-template-${draft.template}`} style={stageStyle}>
        ...
        <div className="thumb-copy">
          ...
          <div className="thumb-title" style={{ fontSize: `${draft.titleSize / 19.2}cqw`, fontWeight: fontWeights[draft.fontStyle] }}>{draft.title || "제목을 입력하세요"}</div>
